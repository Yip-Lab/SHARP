...
class CrossGatingBlock(nn.Module):  # input shape: n, c, h, w
    ...
    def __init__(self, x_features, num_channels, block_size, grid_size, cin_y=128, upsample_y=True, use_bias=True,
...
class MAXIM_dns_3s_hr8_lr4(nn.Module):  # input shape: n, c, h, w
    def __init__(self, features=32, depth=3, use_bias=True, num_supervision_scales=int(3), lrelu_slope=0.2,
                 use_global_mlp=True, high_res_stages=2, block_size_hr=(8, 8), block_size_lr=(4, 4),
                 grid_size_hr=(8, 8), grid_size_lr=(4, 4),
        ...
        self.stage_0_supervised_attention_module_0 = SAM(features=2 ** (0) * self.features,
                                                         output_channels=self.num_outputs, use_bias=self.bias)
        self.enlarge1 = nn.ConvTranspose2d(in_channels=self.num_outputs, out_channels=1, stride=4, kernel_size=4)
        self.enlarge2 = nn.ConvTranspose2d(in_channels=self.num_outputs, out_channels=1, stride=2, kernel_size=2)
    ...
    def forward(self, x):
        ...
        for i in reversed(range(self.depth)):
            if i == 2:
                ...
                outputs3 = torch.mean(output, dim=1)
                outputs3 = outputs3.unsqueeze(1)
                # outputs.append(output)
                # sam_features.append(sam)

        outputs_all = outputs3
